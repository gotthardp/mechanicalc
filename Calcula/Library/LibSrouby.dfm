object Srouby: TSrouby
  Left = 250
  Top = 115
  HelpContext = 1
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = #352'rouby'
  ClientHeight = 121
  ClientWidth = 369
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = True
  Position = poDefaultPosOnly
  PixelsPerInch = 96
  TextHeight = 13
  object LaSroNorm: TLabel
    Left = 8
    Top = 2
    Width = 31
    Height = 13
    Caption = '&Norma'
    FocusControl = EdSroNorm
  end
  object LaSroZav: TLabel
    Left = 8
    Top = 42
    Width = 24
    Height = 13
    Caption = '&Z'#225'vit'
    FocusControl = EdSroZav
  end
  object LaSroKus: TLabel
    Left = 168
    Top = 42
    Width = 24
    Height = 13
    Caption = '&Kus'#367
    FocusControl = EdSroKus
  end
  object LaSroDelka: TLabel
    Left = 88
    Top = 42
    Width = 28
    Height = 13
    Caption = '&D'#233'lka'
    FocusControl = EdSroDelka
  end
  object EdSroNorm: TClrComboBox
    Left = 8
    Top = 16
    Width = 97
    Height = 21
    Hint = 'Norma '#353'roubu'
    Style = csOwnerDrawVariable
    ItemHeight = 16
    Options = [SaveState]
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    OnChange = SroubChange
    OnCreate = EdSroNormCreate
  end
  object EdSroZav: TClrComboBox
    Left = 8
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Velikost z'#225'vitu '#353'roubu'
    Style = csOwnerDrawVariable
    ItemHeight = 16
    Options = [SaveState]
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnChange = SroubChange
    OnColorItem = EdSroZavColorItem
    OnCreate = EdSroZavCreate
  end
  object EdSroKus: TsiEdit
    Left = 168
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Po'#269'et '#353'roub'#367
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'Po'#269'et '#353'roub'#367
    Measure = meNone
    Options = [SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 2
    Text = '0'
    Units = UN_none
    OnChange = EdSroChange
  end
  object EdSroDelka: TClrComboBox
    Left = 88
    Top = 56
    Width = 75
    Height = 21
    Hint = 'D'#233'lka '#353'roubu'
    Style = csOwnerDrawVariable
    ItemHeight = 16
    Options = [SaveState]
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnChange = SroubChange
    OnColorItem = EdSroDelkaColorItem
    OnCreate = EdSroDelkaCreate
  end
  object ScrSro: TScrollBox
    Left = 254
    Top = 0
    Width = 115
    Height = 121
    HorzScrollBar.Visible = False
    Align = alRight
    BorderStyle = bsNone
    TabOrder = 4
    object LaSroHmot: TLabel
      Left = 0
      Top = 2
      Width = 66
      Height = 13
      Caption = '&Hmotnost [kg]'
      FocusControl = EdSroHmot
    end
    object EdSroHmot: TsiComboEdit
      Left = 0
      Top = 16
      Width = 97
      Height = 21
      Hint = 'Celkov'#225' hmotnost'
      ButtonHint = 'Vlo'#382#237' hodnotu do v'#253'razu'
      Caption = '&Hmotnost'
      ColorOK = clWindow
      ColorError = 13172735
      DirectInput = False
      HelpProperties = 4
      HintOK = 'Celkov'#225' hmotnost'
      Glyph.Data = {
        
        320B0FFFFF88033338150BFBFB803333388D0000000333333805}
      ButtonWidth = 18
      Measure = meNone
      NumGlyphs = 1
      Options = [RoundItem, SaveState]
      ParentShowHint = False
      PopupMenu = EdSroHmot.Popup
      Round = roCSN
      RoundUser = 3
      ShowHint = True
      TabOrder = 0
      Text = '0'
      Units = UN_kg
      UnitLabel = LaSroHmot
      OnButtonClick = EdButtonClick
      OnCompute = EdSroHmotCompute
    end
  end
end
