object Trubka: TTrubka
  Left = 227
  Top = 115
  HelpContext = 1
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Trubka'
  ClientHeight = 121
  ClientWidth = 369
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = True
  Position = poDefaultPosOnly
  PixelsPerInch = 96
  TextHeight = 13
  object LaTrMater: TLabel
    Left = 8
    Top = 2
    Width = 37
    Height = 13
    Caption = '&Materi'#225'l'
    FocusControl = EdTrMater
  end
  object LaTrPrumer: TLabel
    Left = 8
    Top = 42
    Width = 50
    Height = 13
    Caption = '&Pr'#367'm'#283'r [m]'
    FocusControl = EdTrPrumer
  end
  object LaTrTloustka: TLabel
    Left = 88
    Top = 42
    Width = 59
    Height = 13
    Caption = '&Tlou'#353#357'ka [m]'
    FocusControl = EdTrTloustka
  end
  object LaTrDelka: TLabel
    Left = 168
    Top = 42
    Width = 45
    Height = 13
    Caption = '&D'#233'lka [m]'
    FocusControl = EdTrDelka
  end
  object EdTrMater: TClrComboBox
    Left = 8
    Top = 16
    Width = 97
    Height = 21
    Hint = 'Materi'#225'l trubky'
    Style = csOwnerDrawVariable
    ItemHeight = 16
    Options = [SaveState]
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    OnChange = EdTrChange
    OnColorItem = EdTrMaterColorItem
    OnCreate = EdTrMaterCreate
  end
  object EdTrPrumer: TsiEdit
    Left = 8
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Vn'#283'j'#353#237' pr'#367'm'#283'r trubky'
    Caption = '&Pr'#367'm'#283'r'
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'Vn'#283'j'#353#237' pr'#367'm'#283'r trubky'
    Measure = meDistance
    Options = [RoundItem, SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 1
    Text = '0'
    Units = UN_m
    UnitLabel = LaTrPrumer
    OnChange = EdTrChange
  end
  object EdTrTloustka: TsiEdit
    Left = 88
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Tlou'#353#357'ka st'#283'ny trubky'
    Caption = '&Tlou'#353#357'ka'
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'Tlou'#353#357'ka st'#283'ny trubky'
    Measure = meDistance
    Options = [RoundItem, SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 2
    Text = '0'
    Units = UN_m
    UnitLabel = LaTrTloustka
    OnChange = EdTrChange
  end
  object EdTrDelka: TsiEdit
    Left = 168
    Top = 56
    Width = 75
    Height = 21
    Hint = 'D'#233'lka trubky'
    Caption = '&D'#233'lka'
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'D'#233'lka trubky'
    Measure = meDistance
    Options = [RoundItem, SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 3
    Text = '0'
    Units = UN_m
    UnitLabel = LaTrDelka
    OnChange = EdTrChange
  end
  object ScrTr: TScrollBox
    Left = 254
    Top = 0
    Width = 115
    Height = 121
    HorzScrollBar.Visible = False
    Align = alRight
    BorderStyle = bsNone
    TabOrder = 4
    object LaTrHmot: TLabel
      Left = 0
      Top = 2
      Width = 66
      Height = 13
      Caption = '&Hmotnost [kg]'
      FocusControl = EdTrHmot
    end
    object LaTrOhyb: TLabel
      Left = 0
      Top = 42
      Width = 48
      Height = 13
      Caption = '&Ohyb [m3]'
      FocusControl = EdTrOhyb
    end
    object LaTrSetr: TLabel
      Left = 0
      Top = 82
      Width = 80
      Height = 13
      Caption = '&Setrva'#269'nost [m4]'
      FocusControl = EdTrSetr
    end
    object EdTrOhyb: TsiComboEdit
      Left = 0
      Top = 56
      Width = 97
      Height = 21
      Hint = 'Moment pr'#367#345'ezu v ohybu k ose x'
      ButtonHint = 'Vlo'#382#237' hodnotu do v'#253'razu'
      Caption = '&Ohyb'
      ColorOK = clWindow
      ColorError = 13172735
      DirectInput = False
      HelpProperties = 4
      HintOK = 'Moment pr'#367#345'ezu v ohybu k ose x'
      Glyph.Data = {
        
        320B0FFFFF88033338150BFBFB803333388D0000000333333805}
      ButtonWidth = 18
      Measure = meNone
      NumGlyphs = 1
      Options = [RoundItem, SaveState]
      ParentShowHint = False
      PopupMenu = EdTrOhyb.Popup
      Round = roCSN
      RoundUser = 3
      ShowHint = True
      TabOrder = 0
      Text = '0'
      Units = UN_m3
      UnitLabel = LaTrOhyb
      OnButtonClick = EdButtonClick
      OnCompute = EdTrOhybCompute
    end
    object EdTrHmot: TsiComboEdit
      Left = 0
      Top = 16
      Width = 97
      Height = 21
      Hint = 'Celkov'#225' hmotnost'
      ButtonHint = 'Vlo'#382#237' hodnotu do v'#253'razu'
      Caption = '&Hmotnost'
      ColorOK = clWindow
      ColorError = 13172735
      DirectInput = False
      HelpProperties = 4
      HintOK = 'Celkov'#225' hmotnost'
      Glyph.Data = {
        
        320B0FFFFF88033338150BFBFB803333388D0000000333333805}
      ButtonWidth = 18
      Measure = meNone
      NumGlyphs = 1
      Options = [RoundItem, SaveState]
      ParentShowHint = False
      PopupMenu = EdTrHmot.Popup
      Round = roCSN
      RoundUser = 3
      ShowHint = True
      TabOrder = 1
      Text = '0'
      Units = UN_kg
      UnitLabel = LaTrHmot
      OnButtonClick = EdButtonClick
      OnCompute = EdTrHmotCompute
    end
    object EdTrSetr: TsiComboEdit
      Left = 0
      Top = 96
      Width = 97
      Height = 21
      Hint = 'Moment setrva'#269'nosti pr'#367#345'ezu k ose x'
      ButtonHint = 'Vlo'#382#237' hodnotu do v'#253'razu'
      Caption = '&Setrva'#269'nost'
      ColorOK = clWindow
      ColorError = 13172735
      DirectInput = False
      HelpProperties = 4
      HintOK = 'Moment setrva'#269'nosti pr'#367#345'ezu k ose x'
      Glyph.Data = {
        
        320B0FFFFF88033338150BFBFB803333388D0000000333333805}
      ButtonWidth = 18
      Measure = meNone
      NumGlyphs = 1
      Options = [RoundItem, SaveState]
      ParentShowHint = False
      PopupMenu = EdTrSetr.Popup
      Round = roCSN
      RoundUser = 3
      ShowHint = True
      TabOrder = 2
      Text = '0'
      Units = UN_m4
      UnitLabel = LaTrSetr
      OnButtonClick = EdButtonClick
      OnCompute = EdTrSetrCompute
    end
  end
end
