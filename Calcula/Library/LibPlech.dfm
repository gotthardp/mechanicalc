object Plech: TPlech
  Left = 279
  Top = 189
  HelpContext = 1
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Plech'
  ClientHeight = 121
  ClientWidth = 369
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = True
  Position = poDefaultPosOnly
  PixelsPerInch = 96
  TextHeight = 13
  object LaPlMater: TLabel
    Left = 8
    Top = 2
    Width = 37
    Height = 13
    Caption = '&Materi'#225'l'
    FocusControl = EdPlMater
  end
  object LaPlPlocha: TLabel
    Left = 88
    Top = 42
    Width = 56
    Height = 13
    Caption = '&Plocha [m2]'
    FocusControl = EdPlPlocha
  end
  object LaPlTloustka: TLabel
    Left = 8
    Top = 42
    Width = 59
    Height = 13
    Caption = '&Tlou'#353#357'ka [m]'
    FocusControl = EdPlTloustka
  end
  object EdPlMater: TClrComboBox
    Left = 8
    Top = 16
    Width = 97
    Height = 21
    Hint = 'Materi'#225'l plechu'
    Style = csOwnerDrawVariable
    ItemHeight = 16
    Options = [SaveState]
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    OnChange = EdPlChange
    OnColorItem = EdPlMaterColorItem
    OnCreate = EdPlMaterCreate
  end
  object EdPlPlocha: TsiEdit
    Left = 88
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Plocha plechu'
    Caption = '&Plocha'
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'Plocha plechu'
    Measure = meNone
    Options = [RoundItem, SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 1
    Text = '0'
    Units = UN_m2
    UnitLabel = LaPlPlocha
    OnChange = EdPlChange
  end
  object EdPlTloustka: TsiEdit
    Left = 8
    Top = 56
    Width = 75
    Height = 21
    Hint = 'Tlou'#353#357'ka plechu'
    Caption = '&Tlou'#353#357'ka'
    ColorOK = clWindow
    ColorError = 13172735
    HelpProperties = 4
    HintOK = 'Tlou'#353#357'ka plechu'
    Measure = meDistance
    Options = [RoundItem, SaveState]
    ParentShowHint = False
    Round = roCSN
    RoundUser = 3
    ShowHint = True
    TabOrder = 2
    Text = '0'
    Units = UN_m
    UnitLabel = LaPlTloustka
    OnChange = EdPlChange
  end
  object ScrPl: TScrollBox
    Left = 254
    Top = 0
    Width = 115
    Height = 121
    HorzScrollBar.Visible = False
    Align = alRight
    BorderStyle = bsNone
    TabOrder = 3
    object LaPlHmot: TLabel
      Left = 0
      Top = 2
      Width = 66
      Height = 13
      Caption = '&Hmotnost [kg]'
      FocusControl = EdPlHmot
    end
    object EdPlHmot: TsiComboEdit
      Left = 0
      Top = 16
      Width = 97
      Height = 21
      Hint = 'Celkov'#225' hmotnost'
      ButtonHint = 'Vlo'#382#237' hodnotu do v'#253'razu'
      Caption = '&Hmotnost'
      ColorOK = clWindow
      ColorError = 13172735
      DirectInput = False
      HelpProperties = 4
      HintOK = 'Celkov'#225' hmotnost'
      Glyph.Data = {
        
        320B0FFFFF88033338150BFBFB803333388D0000000333333805}
      ButtonWidth = 18
      Measure = meNone
      NumGlyphs = 1
      Options = [RoundItem, SaveState]
      ParentShowHint = False
      PopupMenu = EdPlHmot.Popup
      Round = roCSN
      RoundUser = 3
      ShowHint = True
      TabOrder = 0
      Text = '0'
      Units = UN_kg
      UnitLabel = LaPlHmot
      OnButtonClick = EdButtonClick
      OnCompute = EdPlHmotCompute
    end
  end
end
